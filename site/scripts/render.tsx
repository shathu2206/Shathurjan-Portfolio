...
function document(page:React.ReactNode,title:string,description:string,carousel=false){return '<!doctype html>'+renderToStaticMarkup(<html lang="en" style={themeStyle()}><head><script src={`${base}/theme.js`}/><meta charSet="utf-8"/><meta name="viewport" content="width=device-width, initial-scale=1"/><title>{title}</title><meta name="description" content={description}/><link rel="icon" href={`${base}/favicon.svg`}/><link rel="stylesheet" href={`${base}/styles.css`}/><ImageFilters/></head><body>{page}<ThemePicker/><script src={`${base}/site-navigation.js`} defer data-legacy-routes={carousel?JSON.stringify(Object.fromEntries([...routeIds.filter(id=>id!=='intro').map(id=>[id,pageUrl(id)]),...content.projects.map(p=>[p.id,projectUrl(p.id)])])):undefined}/>{carousel&&<script src={`${base}/intro-carousel.js`} defer/>}</body></html>)}
