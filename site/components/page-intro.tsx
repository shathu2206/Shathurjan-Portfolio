...
export function Cover({page}:{page:string}){const c=content.pages.find(p=>p.id===page);if(!c?.cover)return null;return <figure className="page-cover"><img src={asset(c.cover)} alt={c.coverAlt||''} style={{objectPosition:c.position||'center',filter:c.monochrome===false?'none':'grayscale(1)'}} width="1600" height="700"/>{c.credit&&<figcaption>{safeWebUrl(c.creditUrl)?<a href={safeWebUrl(c.creditUrl)} target="_blank" rel="noreferrer">{c.credit}</a>:c.credit}</figcaption>}</figure>}
...
export function OrganizationMark({name,visible=true,logo:customLogo}:{name:string;visible?:boolean;logo?:string}){
  ...
  const logo=source?<img className="organization-logo" src={asset(source)} alt={name} loading="lazy" width="180" height="90"/>:null;
